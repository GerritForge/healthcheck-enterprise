...
public class JVMHeapCheckTest {
  ...
  public void doCheck_returnsPassed() throws Exception {
    ...
    TestMocks mocks = getMocks();

    JVMHeapCheck check =
        new JVMHeapCheck(executor, enterpriseHealthCheckConfig, mocks.metricMaker(), mocks.memoryMXBean());

    when(mocks.memoryUsage().getMax()).thenReturn(100L);
    when(mocks.memoryUsage().getUsed()).thenReturn(1L);

    HealthCheck.Result result = check.doCheck();

    assertEquals(HealthCheck.Result.PASSED, result);
  }
  ...
  public void doCheck_returnsPassWhenMaxUsageUndefined() throws Exception {
    ...
    JVMHeapCheck check =
        new JVMHeapCheck(
            executor, enterpriseHealthCheckConfig, mocks.metricMaker(), mocks.memoryMXBean());

    when(mocks.memoryUsage().getMax()).thenReturn(-1L);
    when(mocks.memoryUsage().getUsed()).thenReturn(90L);
  ...
  public void doCheck_returnsFailWithDefaultThreshold() throws Exception {
    ...
    assertEquals(HealthCheck.Result.FAILED, result);
  }
  ...
  private static TestMocks getMocks() {
    MetricMaker metricMaker = mock(MetricMaker.class);
    MemoryMXBean memoryMXBean = mock(MemoryMXBean.class);
    MemoryUsage memoryUsage = mock(MemoryUsage.class);
    when(memoryMXBean.getHeapMemoryUsage()).thenReturn(memoryUsage);
    return new TestMocks(metricMaker, memoryMXBean, memoryUsage);
  }

  private record TestMocks(
      MetricMaker metricMaker, MemoryMXBean memoryMXBean, MemoryUsage memoryUsage) {}

  private static Config getConfigWithThreshold(int customThreshold) {
