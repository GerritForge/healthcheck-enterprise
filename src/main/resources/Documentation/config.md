...
- `jvm_heap_used`: check JVM Heap usage percentage. `Enabled` by default.
...
- `healthcheck.jvm_heap_used.threshold` :  JVM heap usage percentage above which the check is considered failed.

  Default: 80

_NOTE: This plugin extends the [OSS Health check plugin](https://gerrit.googlesource.com/plugins/healthcheck/)
functionalities. The section above describes only the additional checks and their configurations.
Make sure to check settings and configurations provided by the
[OSS Health check plugin](https://gerrit.googlesource.com/plugins/healthcheck/)._
