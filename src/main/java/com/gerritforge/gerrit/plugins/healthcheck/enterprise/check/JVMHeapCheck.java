...
public class JVMHeapCheck extends AbstractHealthCheck {
  ...
  public Result doCheck() throws Exception {
    ...
    // getMax() may be -1 if undefined, however it should never happen in Gerrit since Xmx is always set
    if (max <= 0) {
    ...
    logger.atSevere().log(
        "Max heap allocated: %d, Heap used %d, Percentage used: %.2f, threshold: %d",
